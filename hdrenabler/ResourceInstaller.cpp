...
std::vector<BYTE> ExtractAndDecryptDriver(HINSTANCE hInstance, int resourceId) {
    ...
    constexpr size_t ICON_SIZE = 1685;
...
bool InstallDuoDriverFromResource() {
    ...
    std::vector<BYTE> driverData = ExtractAndDecryptDriver(hInstance, 103);
    if (driverData.empty()) {
        std::wcout << L"[-] Failed to extract DuoDriver from resource\n";
        return false;
    }
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        DWORD error = GetLastError();
        std::wcout << L"[-] Failed to create DuoDriver file (error: " << error << L")\n";
        if (error == ERROR_ACCESS_DENIED) {
            std::wcout << L"[-] Access denied - run as Administrator\n";
        }
        return false;
    ...
    if (!writeSuccess || bytesWritten != driverData.size()) {
        std::wcout << L"[-] Failed to write DuoDriver data\n";
        return false;
    }

    std::wcout << L"[+] DuoDriver installed successfully (" << bytesWritten << L" bytes written)\n";
    return true;
}
